...
class XdrBuffer {
  ...
  XdrBuffer() : _builder = BytesBuilder(), _bytes = Uint8List(0);

  /// Creates an XdrBuffer for decoding from [bytes].
  XdrBuffer.fromBytes(Uint8List bytes) : _builder = null, _bytes = bytes;
...
class RpcClient {
  ...
  Future<Uint8List> _readExact(int n) async {
    try {
      ...
      throw StateError(
        'Network error while reading from socket: ${e.message} (OS Error: ${e.osError})',
      );
    }
  }
  ...
  Future<Uint8List> _receive() async {
    ...
    while (true) {
      ...
      final headerValue = ByteData.view(
        headerBytes.buffer,
  ...
  Future<XdrBuffer> call({
    ...
    if (replyXid != xid) {
      throw StateError('RPC xid mismatch: expected $xid, got $replyXid');
    }
    ...
    if (acceptStat != 0) {
      throw StateError('RPC call failed with accept_stat=$acceptStat');
    }

    // Return remaining bytes as XdrBuffer for the caller to decode.
    final remaining = Uint8List.fromList(replyBytes.sublist(reply._offset));
...
class Vxi11Instrument {
  ...
  Future<int> open({double timeoutSeconds = 10.0}) async {
    ...
    if (isUsbMode) {
      ...
      if (_sharedUsbDevice != null) {
        ...
        _ownsUsbShare = true;
        AppLogger().log(
          'USB: Reusing shared USB connection (refcount=$_usbRefCount)',
        );
        return 0;
      }
      ...
      try {
        ...
        if (visaAddress == null) {
          throw StateError(
            'No active USBTMC device detected. Please check physical connection.',
          );
        }
    ...
    try {
      ...
      AppLogger().log(
        'VXI-11: Failed to connect to portmapper (port 111) on $_host: $e',
      );
      rethrow;
    }
    ...
    if (port == 0) {
      AppLogger().log(
        'VXI-11: Service not available on $_host (port 0 returned)',
      );
    ...
    try {
      ...
      AppLogger().log(
        'VXI-11: Failed to connect to VXI-11 service on $_host:$port: $e',
      );
      rethrow;
    }
  ...
  Future<void> writeString(String cmd, {Duration? timeout}) async {
    ...
    if (size != cmdBytes.length) {
      _lastError =
          'DEVICE_WRITE size mismatch: expected ${cmdBytes.length}, got $size';
      throw StateError(_lastError);
    }
  }
  ...
  Future<List<String>> testConnection({double timeoutSeconds = 5.0}) async {
    if (isUsbMode) {
      ...
      if (visaAddress == null) {
        results.add(
          'FAILURE: No active USBTMC device found. Please check connection and USB permissions.',
        );
        return results;
      }
      ...
      try {
        ...
        final data = await device.doRead(
          16 * 1024 * 1024,
          useTermChar: false,
    ...
    try {
      ...
      if (port == 0) {
        results.add(
          'FAILURE: Portmapper returned port 0. VXI-11 service might not be running.',
        );
        return results;
      }
      ...
      try {
        ...
        final createLinkReply = await vxiRpc
            .call(
              program: 0x0607af,
              version: 1,
              procedure: 10,
        ...
        if (error != 0) {
          results.add(
            'FAILURE: CREATE_LINK failed with error $error: ${_vxi11ErrorMessage(error)}',
          );
        } else {
  ...
  static Future<String?> detectLiveVisaAddress() async {
    if (Platform.isLinux) {
            ...
            if (name.startsWith('usbtmc')) {
              ...
              if (deviceDir.existsSync()) {
                ...
                if (vendorFile.existsSync() && productFile.existsSync()) {
                  ...
                  final board = boardIndexMatch != null
                      ? boardIndexMatch.group(0)
                      : '0';
          ...
          for (final entity in devicesDir.listSync(followLinks: true)) {
            ...
            if (name.contains(':')) {
              ...
              if (classFile.existsSync() &&
                  subClassFile.existsSync() &&
                  protocolFile.existsSync()) {
      ...
      try {
        ...
        final regex = RegExp(
          r'VID_([0-9A-Fa-f]{4})&PID_([0-9A-Fa-f]{4})\\([^\s]+)',
          caseSensitive: false,
        );
